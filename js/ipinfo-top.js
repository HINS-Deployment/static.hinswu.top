(function() {
  // 防重复运行（SPA/多次注入）
  if (window.__IP_BAR_TOP_INITED__) return;
  window.__IP_BAR_TOP_INITED__ = true;

  const STYLE_ID = 'ip-bar-top-style';
  const BAR_ID = 'ip-bar-top';

  const CACHE_KEY = 'ip_bar_top_cache_v1';
  ...
  const updateVisibility = () => {
    ...
    if (!bar) return;
    // 顶部版本：滚动到顶部附近时显示，滚动下去时隐藏
    bar.classList.toggle('ip-hidden', !isNearTop());
  };
